
import Groq from "groq-sdk";
function getGroqClient() {
  const key = process.env.GROQ_API_KEY;
  
  if (!key) {
    throw new Error("GROQ_API_KEY is missing");
  }
  
  return new Groq({ apiKey: key });
}


export async function routeIntent(query) {
  const groq = getGroqClient();
  const prompt = `
You are an intent classifier for Pratham Tiwari's personal AI portfolio.

Allowed categories:
about, philosophy, education, skills, project, challenge,
journey, interests, achievement, current_focus,
future_goals, why_hire_me,  work_preferences , contact

Rules:
- Return ONLY the most relevant categories
- Use  work_preferences for any work preference profile.
- use contact for any contact profile.
- Prefer specific categories over general ones
- Respond in STRICT JSON ONLY
- Do NOT explain anything

project : ClimaShield, InfinityAR , Alumni
Achievement : Enterpreneurship (ECC) , AlgoArena 

User query:
"${query}"

Output format:
{
  "categories": []
}
`;

  const response = await groq.chat.completions.create({
    model: "llama-3.1-8b-instant",
    messages: [{ role: "user", content: prompt }],
    temperature: 0
  });
 const raw = response.choices[0].message.content;

  try {
    return JSON.parse(raw);
  } catch (err) {
    console.error("INTENT PARSE FAILED:", raw);

    // ✅ SAFE FALLBACK
    return { categories: [] };
  }
}
